...
export class QuestionEffect {
    ...
    fetchQuestions$ = createEffect(() =>
        this.actions$.pipe(
            ...
            exhaustMap(() =>
                this.questionService.getAll().pipe(map((questions) => fetchQuestionsSuccess({ questions })))
            )
        )
    );
        ...
        this.actions$.pipe(
            ...
            exhaustMap(({ question }) =>
                this.questionService.update(question).pipe(
                    map((question) => {
                        this.toastrService.success('Оновлення успішне!');
                        return updateQuestionSuccess({ question });
                    })
                )
            )
        )
