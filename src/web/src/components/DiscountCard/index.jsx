...
const DiscountCard = ({ discount, blocked = false }) => {
    ...
    return (
        <div className={`relative rounded-standart  my-5 h-22 py-3 items-center ${blocked ? 'bg-[#C1C1C1]' : 'bg-[#9AE19D]'}`}>
            <div className='w-11/12 flex justify-between items-center mx-auto'>
                <div>
                    <p className='text-bold text-lg flex justify-between mb-0 items-center'>{name}</p>
                    {!blocked && (
                        <span className='text-black/60 max-sm:text-xs font-normal'>
                            ...
                            {withBarCode && withQrCode && 'Цю знижку можна сканувати будь яким способом'}
                        </span>
                    )}
                    {blocked && (
                ...
                <div className=' flex justify-between gap-2'>
                    {!blocked && (
                        <React.Fragment>
                            ...
                            {withQrCode && (
                                <Button onClick={toggleQrCodeModal} className='!h-12 !w-12 text-2xl px-3'>
                ...
                {isQrCodeModalOpen && <DiscountCardModal discount={discount} close={toggleQrCodeModal} />}
                {isBarCodeOpen && <DiscountCardModal discount={discount} close={toggleBarCodeModal} isQrDiscount={false} />}
                {isInfoModalOpen && <DiscountInfoModal close={toggleInfoModal} discount={discount} />}
            </div>
            {}
        </div>
    );
        ...
        <SimpleModal className='w-[320px] !h-fit max-h-[80vh]'>
            <div>
                ...
                {discount.description.length < 100 && (
                    <div className='text-ellipsis overflow-hidden' dangerouslySetInnerHTML={{ __html: discount?.description }}></div>
                )}
                <div className='my-8 w-full max-xs:!my-4'>
                    {loading ? (
                        ...
                        <div className='grid place-items-center h-fit'>
                            {isQrDiscount ? (
                                <div className='mb-3'>
                                    <QrcodeGenerator size={isSmScreen ? 200 : 250} value={qrCodeValue} />
                                </div>
                            ) : (
                                <React.Fragment>
                                    <div className=''>
                                        <img
                                            ...
                                            alt=''
                                            className={isSmScreen ? 'h-[150px] w-[250px]' : 'h-[250px] w-[350px]'}
                                        />
                                    </div>
                                </React.Fragment>
                            )}
                            <Timer finishHandler={loadDiscount} isTimerStarted={isTimerStarted} setIsTimerStarted={setIsTimerStarted} />
                        </div>
                    )}
                </div>
